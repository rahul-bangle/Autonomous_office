...
class Supervisor:
    ...
    def _handle_exit(self, sig, frame) -> None:
        ...
        self._send_native_toast("Office OS Supervisor", "🚨 Office OS Supervisor Stopped")
...
"""
    ...
    def run(self, once: bool = False) -> None:
        self._console("=" * 62)
        self._console("OFFICE OS — AUTONOMOUS SUPERVISOR")
        self._console(
        ...
        # Send startup toast
        self._send_native_toast("Office OS Supervisor", "[OK] Office OS Supervisor Started")
        ...
        try:
            ...
            self._send_native_toast("Office OS Supervisor", "[ALERT] Office OS Supervisor Stopped")
    ...
    def _log_loop_to_master_context(
        ...
        *,
        ...
        entry = (
            f"[{started_at.strftime('%Y-%m-%d %H:%M:%S')}] CMD: `py -3.11 supervisor.py`\n"
            f"CONTEXT: Supervisor autonomous loop cycle {self.cycle}.\n"
            ...
            f"STATUS: {'[OK] complete' if health_ok else '⚠ degraded'}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run Office OS Supervisor watchdog")
    parser.add_argument(
        "--once",
        action="store_true",
